...
1. Projects are completely unrelated to task types. Each session needs to be attached to a project because the project contains the git repository information.
2. Project ID is custom, can use natural language to make it readable, but cannot be changed in the future, can only change the name.

```bash
# Create a project
kanbatte session project create -p project-id -n project-name -r git@github.com:example/example.git -d description
kanbatte session project create --project project-id --name project-name --repository git@github.com:example/example.git --description description

# Update a project
kanbatte session project update -p project-id -n project-name -r git@github.com:example/example.git -d description

# List projects (shows table with columns: project-id, project-name, repository, description)
