...
export const agent = async (deps) => {
  ...
  const readySessions = await sessionService.getSessionsByStatus({
    status: "ready",
  });
  ...
  for (const session of readySessions) {
    ...
    try {
      ...
      const project = sessionService.getProjectById({
        projectId: session.project,
      });
      if (!project || !project.gitRepository) {
        throw new Error(
          `No repository found for project ${session.project} in config file.`,
        );
      }
      ...
      if (session.promptPreset) {
        ...
        if (systemPrompt) {
          ...
          console.warn(
            `Prompt preset '${session.promptPreset}' not found in config. Using default.`,
          );
        }
      }
      ...
      try {
        ...
        for await (const message of result) {
          const currentSessionState = await sessionService.getViewBySessionId({
            sessionId: session.sessionId,
          });
          if (currentSessionState.status !== "in-progress") {
            console.warn(
              `Interruption detected for session ${session.sessionId}.`,
            );
            break;
          }
          if (
            ...
            console.log(
              `Claude session started and saved for ${session.sessionId}`,
            );
          }
      ...
      if (currentSessionState.status !== "in-progress") {
        console.warn(
          `Work on session ${session.sessionId} was interrupted by user. Discarding results.`,
        );
        continue;
      }
